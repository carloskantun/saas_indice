<?php
/**
 * /modules/processes_tasks/includes/business.php
 * Retorna lista de negocios para selector
 * 
 * Parámetros opcionales:
 * - unidad: Filtra negocios por unidad
 */
...
if (!hasPermission($ucId, 'processes_tasks', 'view')) {
    $logLine = json_encode([
        'ts' => date('c'),
        'endpoint' => 'includes/business.php',
...
try {
    $unidad = isset($_GET['unidad']) ? trim($_GET['unidad']) : null;
    ...
    $query = "
        SELECT DISTINCT negocio AS label, negocio AS value
        FROM tasks
